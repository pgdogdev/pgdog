...
async fn test_abrupt_disconnect() {
    ...
    let event = client.buffer(State::Idle, None).await.unwrap();
...
async fn test_client_idle_timeout() {
    ...
    let res = client.buffer(State::Idle, None).await.unwrap();
    ...
    assert!(
        timeout(
            ...
            client.buffer(State::IdleInTransaction, None)
        )
        .await
        .is_err()
    );
...
async fn test_query_timeout() {
    ...
    client.buffer(State::Idle, None).await.unwrap();
