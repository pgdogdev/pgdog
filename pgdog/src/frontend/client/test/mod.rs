...
async fn test_client_idle_in_transaction_timeout() {
    ...
    let start = Instant::now();
    let res = client.buffer(State::IdleInTransaction).await.unwrap();
    assert_eq!(res, BufferEvent::IdleInTransactionTimeout);

    let err = read_one!(conn);
    assert!(start.elapsed() >= Duration::from_millis(25));
    let err = ErrorResponse::from_bytes(err.freeze()).unwrap();
    assert_eq!(err.code, "57P05");
    assert!(err.message.contains("idle in transaction"));
}

#[tokio::test]
async fn test_client_idle_in_transaction_timeout_pool_stat() {
    crate::logger();
    load_test();
    ...
    let mut config = (*config()).clone();
    config.config.general.client_idle_in_transaction_timeout = 50;
    set(config).unwrap();
    ...
    // BEGIN is virtual until a query checks out a backend; the timeout
    // counter is recorded on the pool holding that checkout.
