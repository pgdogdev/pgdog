...
struct Global {
    shutdown: Arc<Notify>,
    traffic_is_paused: AtomicBool,
    traffic_unpaused_notif: Arc<Notify>,
    offline: AtomicBool,
...
impl Comms {
    ...
    fn new() -> Self {
        Self {
            global: Arc::new(Global {
                ...
                traffic_is_paused: false.into(),
                traffic_unpaused_notif: Arc::new(Notify::new()),
    ...
    /// Traffic is/should be paused.
    pub fn is_traffic_paused(&self) -> bool {
        self.global.traffic_is_paused.load(Ordering::Relaxed)
    }

    /// Pause traffic to all clients.
    pub fn pause_traffic(&self) {
        self.global.traffic_is_paused.store(true, Ordering::Relaxed);
    }

    /// Unpause traffic to all clients.
    pub fn unpause_traffic(&self) {
        self.global
            .traffic_is_paused
            .store(false, Ordering::Relaxed);
        self.global.traffic_unpaused_notif.notify_waiters();
    }

    /// Get the `Notify` to wait for unpausing traffic.
    pub fn unpausing_traffic(&self) -> Arc<Notify> {
        self.global.traffic_unpaused_notif.clone()
    }
}
