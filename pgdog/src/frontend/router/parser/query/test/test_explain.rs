...
fn test_explain_select() {
    ...
    let command = test.execute(vec![
        Query::new("EXPLAIN SELECT * FROM sharded WHERE id = 1").into(),
    ]);
...
fn test_explain_analyze_select() {
    ...
    let command = test.execute(vec![
        Query::new("EXPLAIN ANALYZE SELECT * FROM sharded WHERE id = 1").into(),
    ]);
...
fn test_explain_insert() {
    ...
    let command = test.execute(vec![
        Query::new("EXPLAIN INSERT INTO sharded (id, email) VALUES (1, 'test')").into(),
    ]);
...
fn test_explain_with_comment_shard_override() {
    ...
    let command = test.execute(vec![
        Query::new("/* pgdog_shard: 5 */ EXPLAIN SELECT * FROM sharded").into(),
    ]);
...
fn test_explain_verbose() {
    ...
    let command = test.execute(vec![
        Query::new("EXPLAIN (VERBOSE, COSTS) SELECT * FROM sharded WHERE id = 1").into(),
    ]);
