...
pub struct General {
    ...
    ///
    /// **Note:** If this limit is reached, the statement chosen by `prepared_statements_eviction` is closed and replaced with the newest one. Additionally, any unused statements in the global cache above this limit will be removed.
    ///
    ...
    /// Which statement a server connection closes once it reaches `prepared_statements_limit`.
    ///
    /// **Note:** `lfu` suits workloads with many statements where a few are much hotter than the rest; `lru` suits workloads that move through statements in phases.
    ///
    /// _Default:_ `lru`
    ///
    ...
    ///
    /// <https://docs.pgdog.dev/configuration/pgdog.toml/general/#prepared_statements_ttl_jitter>
    #[serde(default = "General::default_prepared_statements_ttl_jitter")]
    pub prepared_statements_ttl_jitter: u64,
...
impl General {
    ...
    /// How long a statement can stay prepared on a server connection.
    ///
    ...
    pub fn default_prepared_statements_ttl_jitter() -> u64 {
        Self::env_or_default(
            "PGDOG_PREPARED_STATEMENTS_TTL_JITTER",
...
mod tests {
    ...
    fn test_prepared_statements_ttl_jitter_never_reaches_the_ttl() {
        for ttl in [0, 1, 2, 300_000] {
            ...
            let jitter = general.prepared_statements_ttl_jitter().as_millis() as u64;
            assert!(
                jitter < ttl.max(1),
                "jitter {jitter} must stay below ttl {ttl}"
            );
        }
    }
    ...
    fn test_env_enum_fields() {
        ...
        // Test prepared statements eviction
