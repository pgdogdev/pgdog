...
impl RuntimeConfig {
    ...
    pub(crate) fn from_settings(mut settings: Settings) -> Result<Self, ConfigError> {
        ...
        // Without an audience allow-list, any Google access token for any
        // OAuth client logs its owner in: a token the user granted to an
        // unrelated application would authenticate them against this database
        // (token confusion). The audience is what binds a token to this
        // deployment, so there is no usable default.
        if settings.allowed_audiences.is_empty() {
            return Err(ConfigError::Invalid(
                "allowed_audiences is required: list the OAuth client IDs (the \"aud\"/\"azp\" claims) whose access tokens may log in".into(),
            ));
        }
...
mod tests {
    ...
    /// Settings that load: `allowed_audiences` is mandatory, so every test
    /// that expects `from_settings` to succeed has to set it.
    ...
    #[test]
    fn requires_an_audience_allowlist() {
        let error = RuntimeConfig::from_settings(Settings::default())
            .err()
            .expect("an empty allowed_audiences must not load");
        assert!(error.to_string().contains("allowed_audiences is required"));
    }
...
"#,
        ...
        assert_eq!(runtime.settings.allowed_audiences, ["gcloud-client"]);
