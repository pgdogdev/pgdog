...
pub trait Plugin {
    ...
    /// # Concurrency
    ///
    /// PgDog calls this from its blocking thread pool, so clients can be
    /// authenticated concurrently and on different threads; implementations
    /// must be thread-safe. Unlike [`Plugin::route`], blocking I/O is expected
    /// here (an HTTP round trip to an identity provider, for example) and does
    /// not stall PgDog's runtime. Apply your own timeout: a call that never
    /// returns holds on to a thread in that pool.
    ///
    /// # Panics
    ///
    /// A panic is caught before it can unwind across the FFI boundary and is
    /// turned into [`AuthDecision::Deny`], so a bug here rejects one client
    /// instead of aborting PgDog.
        ...
        match decision {
            ...
            AuthDecision::Deny(reason) => {
                ...
                AuthOutcome {
                    ...
                    read_only: AuthOutcome::READ_ONLY_UNSET,
                    provision: false,
                }
            }
...
impl PluginVtable {
    ...
    /// # Panics
    ///
    /// `on_field` must not panic. It is reached from the plugin through an
    /// `extern "C-unwind"` trampoline, and an unwind back out of the shared
    /// library is a foreign exception that aborts the process instead of
    /// being caught. Copy the value here and do fallible work with it after
    /// this call returns.
