...
use rust::setup::{admin_sqlx, admin_tokio, connection_failover};
...
async fn test_maintenance_mode_multiple_cycles() {
    ...
    for i in 0..3 {
        ...
        conn.execute(format!("SELECT {}", i * 2 + 1).as_str()).await.unwrap();
        ...
        conn.execute(format!("SELECT {}", i * 2 + 2).as_str()).await.unwrap();
    }
